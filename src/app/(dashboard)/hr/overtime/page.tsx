...
import { countByStatus, findPendingOvertimeRequests } from '@/domains/hr/repositories/overtime-repository'
...
export default async function OvertimePage() {
  ...
  const [pending, counts] = await Promise.all([
    findPendingOvertimeRequests(tenantId, 20),
  ...
  return (
    <section>
      <PageHeader
        ...
        description="Review and approve employee overtime requests with cost estimates."
      />
        ...
        <div className="rounded-xl border bg-card text-card-foreground shadow-sm p-8">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-lg bg-secondary flex items-center justify-center">
              <i className="fa-solid fa-hourglass text-amber-600 fa-lg"></i>
            </div>
      ...
      {/* Pending Overtime Requests List */}
      <OvertimeList requests={pending} isApprover={true} />
    </section>
  )
}
